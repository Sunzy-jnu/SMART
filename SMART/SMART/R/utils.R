...
#'
#' @param vec_1 a numeric vector representing the first vector.
#' @param vec_2 a numeric vector representing the second vector.
#'
#' @return a numeric value representing the cosine similarity between the two vectors.
#' @export
cosine_similarity_vec <- function(vec_1, vec_2) {
  vec_1 <- as.numeric(vec_1)
  vec_2 <- as.numeric(vec_2)

  dot_product <- sum(vec_1 * vec_2)

  norm_vec_1 <- sqrt(sum(vec_1^2))
  norm_vec_2 <- sqrt(sum(vec_2^2))
