...
#' @param spatial_coords a matrix indicating the spatial coordinates of spots (spot x 2).
...
#' @param lambda_spatial a value indicating the weight of the spatial smoothness term. The default is 0.8.
#' @param neighbor_radius a value indicating the radius to define spatial neighbors. The default is 5.
...
#' @import peakRAM
st_train <- function(st_counts, sc_results, spatial_coords, n_hvgs = 5000, lambda_spatial = 0.8, neighbor_radius = 5, sigma = 0.3, tol = 1e-8, maxiter = 2000, save_res = TRUE, save_mu_csv = FALSE, save_dir = ".") {
  ...
  st_peak <- peakRAM(
    ...
    st_results <- st_model(Y = Y, N = N, phi = phi, Q = Q, spatial_coords = spatial_coords, lambda_spatial = lambda_spatial, neighbor_radius = neighbor_radius, sigma = sigma, tol = tol, maxiter = maxiter),
  ...
  if (save_res) {
    ...
    write.csv(as.matrix(st_results$proportion), file = paste(save_dir, "SMART_prop.csv", sep = "/"))
    write.csv(as.matrix(st_results$alpha), file = paste(save_dir, "SMART_alpha.csv", sep = "/"))
  }

  if (save_mu_csv) {
    for (i in names(st_results$mu)) {
      write.csv(as.matrix(st_results$mu[[i]]), file = paste(save_dir, paste(c("SMART", i, "mu.csv"), collapse = "_"), sep = "/"))
    }
  }
...
#' The core function for deconvolution with spatial smoothing
#'
...
st_model <- function(Y, N, phi, Q, spatial_coords, lambda_spatial = 0.8, neighbor_radius = 5, sigma = 0.3, tol = 1e-8, maxiter = 2000) {
  ...
  W_spatial <- exp(-dist_matrix^2 / (2 * neighbor_radius^2))
  diag(W_spatial) <- 0

  loss_func <- function(Y, N, phi, omega, gamma, sigma, W_spatial, lambda_spatial) {
    n_spots <- dim(Y)[1]
    n_genes <- dim(Y)[2]
    temp <- omega %*% phi
    loss_expression <- -sum(diag(t(Y) %*% log((N %*% t(rep(1, n_genes))) * temp * (rep(1, n_spots) %*% t(exp(gamma))) + eps))) +
      t(N) %*% temp %*% exp(gamma) +
    ...
    for (i in 1:n_spots) {
      for (j in 1:n_spots) {
        if (i != j && W_spatial[i, j] > 0) {
          loss_spatial <- loss_spatial + W_spatial[i, j] * sum((beta[i, ] - beta[j, ])^2)
        }
      }
    }
    loss_spatial <- lambda_spatial * loss_spatial / (n_spots * (n_spots - 1))

    return(as.numeric(loss_expression + loss_spatial))
  }
  ...
  for (iter in 1:maxiter) {
    ...
    if (iter != 1) {
      loss <- loss_func(Y, N, phi, omega, gamma, sigma, W_spatial, lambda_spatial)
      if (abs(loss - loss_old) / abs(loss_old) < tol) {
        break
      }
      loss_old <- loss
    } else {
      loss_old <- loss_func(Y, N, phi, omega, gamma, sigma, W_spatial, lambda_spatial)
    }
  }
