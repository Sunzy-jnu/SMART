...
#################### Preprocess data downloaded from TISCH

### The original data can be downloaded from http://tisch.comp-genomics.org/gallery/?cancer=NSCLC&species=Human (NSCLC_EMTAB6149).

TISCH_preprocess(files_path = "SMART_data/paired_scenario/NSCLC_EMTAB6149", n_cells_per_type = NULL, output_dir = "SMART_data/paired_scenario", do_return = FALSE, prefix = "")

#################### Split the dataset into a generation dataset and a validation dataset
sc_counts_path <- "SMART_data/paired_scenario/counts.csv"
sc_labels_path <- "SMART_data/paired_scenario/labels.csv"
...
split_dataset(sc_counts, sc_labels, min_cells_per_type = 30, save_dir = "SMART_data/paired_scenario/sc_data")



#################### Visualize the generation dataset and the validation dataset

sc_counts_generation_path = "SMART_data/paired_scenario/sc_data/counts_generation.csv"
sc_labels_generation_path = "SMART_data/paired_scenario/sc_data/labels_generation.csv"
sc_counts_validation_path = "SMART_data/paired_scenario/sc_data/counts_validation.csv"
sc_labels_validation_path = "SMART_data/paired_scenario/sc_data/labels_validation.csv"
...
saveRDS(plot_df, file = "SMART_results/simulation/paired_scenario/visualization/visualization_plot_df.rds")

library(ggplot2)
library(ggprism)

pdf(paste("SMART_results/simulation/paired_scenario/visualization", paste("UMAP_labels", "pdf", sep = "."), sep = "/"), width = 6, height = 4)
fig <- ggplot(data = plot_df, aes(x = dim1, y = dim2, color = as.factor(label))) +
  geom_point(size = 0.2) +
  # coord_equal() +
  coord_fixed(ratio = 0.9393934) +
...
pdf(paste("SMART_results/simulation/paired_scenario/visualization", paste("UMAP_domain", "pdf", sep = "."), sep = "/"), width = 6, height = 4)
...
sc_counts_path <- "SMART_data/paired_scenario/sc_data/counts_generation.csv"
sc_labels_path <- "SMART_data/paired_scenario/sc_data/labels_generation.csv"
...
for (setting in names(simulation_settings)) {
  ...
  save_dir <- file.path("SMART_data/paired_scenario/st_data", setting)
  generate_pseudo_st(
    sc_counts = sc_counts,
    sc_labels = sc_labels,
    coords = coords,
    n_spots = 1000,
    min_cells = simulation_settings[[setting]]$min_cells,
    max_cells = simulation_settings[[setting]]$max_cells,
    temperature = simulation_settings[[setting]]$temperature,
    tech_type = simulation_settings[[setting]]$tech_type,
    save_dir = save_dir
  )
}
...
library(SMART)
sc_counts_path <- "SMART_data/paired_scenario/sc_data/counts_validation.csv"
sc_labels_path <- "SMART_data/paired_scenario/sc_data/labels_validation.csv"
...
sc_results <- sc_train(sc_counts, sc_labels, n_archetypes_vec = 10, n_cores = 6, save_dir = "SMART_results/simulation/paired_scenario/sc_results")

for (i in list.files("SMART_data/paired_scenario/st_data")){

  st_counts_path <- file.path("SMART_data", "paired_scenario", "st_data", i, "st_counts.csv")
  st_coords_path <- file.path("SMART_data", "paired_scenario", "st_data", i, "st_coords.csv")

    st_counts <- as.matrix(read.csv(st_counts_path, header = TRUE, row.names = 1))
    st_coords <- as.matrix(read.csv(st_coords_path, header = TRUE, row.names = 1))
  ...
  st_counts <- st_counts[, common_genes]
    ...
    st_results <- st_train(
      ...
      spatial_coords = st_coords,
      lambda_spatial = 0.8,
      neighbor_radius = 5,  
      k_neighbors = 10,         
      adaptive_sigma = 1.2,      
      save_mu_csv = TRUE,
      save_dir = paste("SMART_results/simulation/paired_scenario/st_results", i, sep = "/"))
}
...
for (suffix in suffix_all){
  ...
  truth_dir <- paste("D:/Rshuju/shiyan/SMART_data/paired_scenario/st_data", suffix, sep = "/")
  results_dir <- list(SMART = paste("D:/Rshuju/shiyan/SMART_results/simulation/paired_scenario/st_results", suffix, sep = "/"),
                      PANDA = paste("D:/Rshuju/shiyan/SMART_results/simulation/paired_scenario/comparison_methods/PANDA_results", suffix, sep = "/"),
                      cell2location = paste("D:/Rshuju/shiyan/SMART_results/simulation/paired_scenario/comparison_methods/cell2location_results", suffix, sep = "/"),
                      DestVI = paste("D:/Rshuju/shiyan/SMART_results/simulation/paired_scenario/comparison_methods/DestVI_results", suffix, sep = "/"),
                      RCTD = paste("D:/Rshuju/shiyan/SMART_results/simulation/paired_scenario/comparison_methods/RCTD_results", suffix, sep = "/"),
                      SPOTlight = paste("D:/Rshuju/shiyan/SMART_results/simulation/paired_scenario/comparison_methods/SPOTlight_results", suffix, sep = "/"),
                      stereoscope = paste("D:/Rshuju/shiyan/SMART_results/simulation/paired_scenario/comparison_methods/stereoscope_scvi_results", suffix, sep = "/"))
  output_dir <- paste("D:/Rshuju/shiyan/SMART_results/simulation/paired_scenario/comparison_results", suffix, sep = "/")

  if (!dir.exists(output_dir)){
    dir.create(output_dir, recursive = TRUE)
  }

  truth_file <- list.files(truth_dir, pattern = "st_type_prop.csv", full.names = TRUE)
  if (length(truth_file) == 0) {
    stop("Truth file not found in: ", truth_dir)
  }
  type_prop_truth <- read.csv(truth_file, header = TRUE, row.names = 1)
  ...
  cat("Cell type proportion evaluation:\n")
  cat("PCC values:\n")
  print(prop_pearson_spot_list)
  cat("RMSE values:\n")
  print(prop_RMSE_spot_list)
  cat("JSD values:\n")
  print(prop_JSD_spot_list)
  ...
  cat("Cell type signature evaluation:\n")
  cat("PCC values:\n")
  print(signature_pearson_spot_list)
  cat("RMSE values:\n")
  print(signature_RMSE_spot_list)
  cat("JSD values:\n")
  print(signature_JSD_spot_list)
  ...
  plot_prop_signature_acc(prop_signature_pearson, fname = "prop_signature_pearson", save_dir = output_dir, size = 0.5, color = "deepskyblue3", y_lab = "Pearson correlation coefficient (PCC) between estimated cell type signature and ground truth", n_cols = 3, x_breaks = seq(0, 1, by = 0.1), width = 10, height = 11)
  plot_prop_signature_acc(prop_signature_RMSE, fname = "prop_signature_RMSE", save_dir = output_dir, size = 0.5, color = "deepskyblue3", y_lab = "Root mean squared error (RMSE) between estimated cell type signature and ground truth", n_cols = 3, x_breaks = seq(0, 1, by = 0.1), width = 10, height = 11)
  plot_prop_signature_acc(prop_signature_JSD, fname = "prop_signature_JSD", save_dir = output_dir, size = 0.5, color = "deepskyblue3", y_lab = "Jensen-Shannon divergence (JSD) between estimated cell type signature and ground truth", n_cols = 3, x_breaks = seq(0, 1, by = 0.1), width = 10, height = 11)
...
saveRDS(integrated_results_list, file = "SMART_results/simulation/paired_scenario/comparison_results/integrated_results_list.rds")
...
write.csv(rank_matrix_prop, file = "SMART_results/simulation/paired_scenario/comparison_results/rank_matrix_prop.csv")
write.csv(rank_matrix_signature, file = "SMART_results/simulation/paired_scenario/comparison_results/rank_matrix_signature.csv")

mean_rank_prop <- colMeans(rank_matrix_prop)
mean_rank_signature <- colMeans(rank_matrix_signature)


plot_rank(mean_rank_prop, mname = all_methods, use_color = use_color, fname = "mean_rank_prop", save_dir = "SMART_results/simulation/paired_scenario/comparison_results", width = 7.5, height = 4)
plot_rank(mean_rank_signature, mname = all_methods, use_color = use_color, fname = "mean_rank_signature", save_dir = "SMART_results/simulation/paired_scenario/comparison_results", width = 7.5, height = 4)
...
for (i in suffix_all){
  ...
  fname <- i
  pdf(paste("SMART_results/simulation/paired_scenario/comparison_results", paste(fname, "pdf", sep = "."), sep = "/"), width = 7.5, height = 3.8)
